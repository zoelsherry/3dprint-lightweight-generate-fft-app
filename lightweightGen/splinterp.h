...
void lineSpace(value_t* line, value_t begin, value_t end, int inter_num)
{
...
template<typename value_t>
void meshGrid(value_t* xx, value_t* yy, value_t* zz, value_t* x, value_t* y, value_t* z, int length_x, int length_y, int length_z)
{
	for (int i = 0; i < length_z; i++)
		for (int j = 0; j < length_x; j++)
			for (int k = 0; k < length_y; k++)
				xx[i*length_x*length_y + j * length_y + k] = x[j];


	for (int i = 0; i < length_z; i++)
		for (int j = 0; j < length_x; j++)
			for (int k = 0; k < length_y; k++)
				yy[i*length_x*length_y + j * length_y + k] = y[k];

	for (int i = 0; i < length_z; i++)
		for (int j = 0; j < length_x; j++)
			for (int k = 0; k < length_y; k++)
				zz[i*length_x*length_y + j * length_y + k] = z[i];
